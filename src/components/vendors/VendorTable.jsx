...
export default function VendorTable({
  ...
  useEffect(() => {
    ...
    if (externalVendors.length > 0) {
      console.log('📊 VendorTable received externalVendors:', externalVendors.length, 'vendors')
      console.log('   First vendor:', externalVendors[0])
    }
  }, [externalVendors.length, internalVendors.length, externalVendors])

  const allVendors = useMemo(() => {
    const internal = internalVendors.map(v => ({ ...v, source: 'internal' }))
    const external = externalVendors.map(v => {
      console.log('📊 Mapping external vendor:', v)
      return {
        id: v.name,
        name: v.name,
        type: 'External',
    ...
    const result = activeTab === 'internal' ? internal : external
    console.log('📊 allVendors for activeTab=' + activeTab + ':', result.length, 'vendors')
    return result
  ...
  const filteredVendors = useMemo(() => {
    ...
    console.log('📊 filteredVendors:', filtered.length, 'vendors (search="' + searchQuery + '", status="' + statusFilter + '")')
    return filtered
  ...
  const isVendorSelected = (vendor) => {
    return selectedForRfq.some(v => v.id === vendor.id && v.name === vendor.name)
  }

  // Check if no vendors exist at all
  const hasNoVendors = internalVendors.length === 0 && externalVendors.length === 0
    ...
    <div className="flex flex-col h-full">
      ...
      {hasNoVendors ? (
        <div className="flex-1 flex flex-col items-center justify-center p-8">
          <div className="text-center max-w-sm">
            <div className="w-16 h-16 bg-lyzr-light-2 rounded-full flex items-center justify-center mx-auto mb-4">
              <Building2 className="w-8 h-8 text-lyzr-mid-4" />
            </div>
            ...
            <p className="text-sm text-lyzr-mid-4">
              No vendors are currently available matching your search criteria. Try adjusting your filters or search terms.
            </p>
          </div>
        </div>
      ) : (
        <>
          ...
          {/* Send RFQ Button - appears when vendors are selected */}
          <AnimatePresence>
                ...
                <div className="p-4">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-sm text-lyzr-congo">
                      Selected: <strong>{selectedForRfq.length}</strong> vendor{selectedForRfq.length !== 1 ? 's' : ''}
                    </span>
                  </div>
                  <div className="flex gap-2 mb-3 flex-wrap">
                    {selectedForRfq.map(vendor => (
                      <Badge key={`${vendor.id}-${vendor.name}`} variant="info" size="sm">
                        {vendor.name}
                      </Badge>
                    ))}
                  </div>
                  ...
                  >
                    ...
                    Send RFQ to {selectedForRfq.length} Vendor{selectedForRfq.length !== 1 ? 's' : ''}
                  </Button>
                </div>
              </motion.div>
            )}
