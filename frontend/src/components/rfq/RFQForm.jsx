...
export default function RFQForm({ rfqData }) {
  ...
  useEffect(() => {
    if (rfqData) {
      ...
      if (rfqData.additionalFields?.length > 0) {
        setAdditionalFields(rfqData.additionalFields.map(f => ({
      ...
      const sectionsToExpand = ['general', 'procurement', 'timeline']
      if (rfqData.additionalFields?.length > 0) {
  ...
  const getOverallCompletion = () => {
    ...
    const totalFields = allFields.length + additionalFields.length
    const filledAdditional = additionalFields.filter(f => f.field_value?.trim()).length
    return Math.round(((filledFields.length + filledAdditional) / totalFields) * 100)
  }
  ...
  const handleGenerateRfq = async (e) => {
    ...
    try {
      const result = await generateRfqDocument({
        ...formData,
              ...
              >
                <div className="px-4 pb-4 pt-2 space-y-4 border-t border-lyzr-light-2">
                  {additionalFields.map((field, idx) => {
                    ...
                    // Use textarea for long values (>80 chars) or multiline content
