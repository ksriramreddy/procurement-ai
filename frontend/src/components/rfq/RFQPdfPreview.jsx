...
import { Send, Download, Edit3, Save, X, Eye, FileText, Loader, Search } from 'lucide-react'
...
function sanitizeForPdf(text) {
  return text
    ...
    .replace(/[^\x00-\xFF]/g, '')                              // strip any remaining non-WinAnsi chars
}

export default function RFQPdfPreview({ rfqDocument, documentType = 'RFQ' }) {
  const { currentChat, currentChatId, addMessage, setPendingChatMessage, hideDetailPanel } = useChatStore()
  ...
  const generatePdf = async (text) => {
    try {
      ...
      // Strip HTML if present, then sanitize for WinAnsi encoding
      const plainText = sanitizeForPdf(
        /<[a-z][\s\S]*>/i.test(text) ? stripHtml(text) : text
      )
  ...
  // Extract an integer price from the budget string (e.g. "50 million" → 50000000, "$5,000" → 5000)
  ...
  // Build snake_case form data for the certification agent from camelCase store data
  const buildCertAgentInput = () => {
    const rfq = currentChat?.rfqData
    const rfp = currentChat?.rfpData
    if (documentType === 'RFP' && rfp) {
      return {
  ...
  const handleSend = async () => {
    ...
    // Determine subject & quoted price based on document type
    let subject = ''
    let quotedPrice = null
    if (documentType === 'RFP') {
      subject = currentChat?.rfpData?.projectTitle || 'RFP Request'
      quotedPrice = parseBudgetToInt(currentChat?.rfpData?.budget)
    } else if (documentType === 'Contract') {
      subject = currentChat?.contractData?.scope || 'Contract'
    } else {
      subject = currentChat?.rfqData?.requirementSummary || 'RFQ Request'
      quotedPrice = parseBudgetToInt(currentChat?.rfqData?.budgetRange)
    }
    ...
    if (documentType === 'RFQ' || documentType === 'RFP') {
      const certInput = buildCertAgentInput()
      if (certInput) {
        try {
          certificationData = await callCertificationAgent(certInput, currentChat?.sessionId)
        } catch (err) {
          console.warn('[RFQPdfPreview] certification agent failed:', err.message)
        }
      }
    }
    ...
    if (allVendors.length > 0) {
      try {
        const sendResult = await sendDocumentToVendors({
          ...
          subject,
          quotedPrice,
          documentContent: editableContent || null,
          certificationData
        })

        // Step 3: Upload PDF to S3 for each vendor thread
        const allResults = [...(sendResult.created_vendors || []), ...(sendResult.updated_vendors || [])]
        if (allResults.length > 0) {
          // Generate PDF blob from content
          const pdfBlob = await generatePdf(editableContent)
          if (pdfBlob) {
            ...
            await Promise.all(
              allResults.map(v =>
                ...
                }).catch(err => console.warn(`[RFQPdfPreview] S3 upload failed for ${v.vendor_name}:`, err.message))
              )
            )
          }
        }
      } catch (err) {
        console.warn('[RFQPdfPreview] sendDocumentToVendors failed:', err.message)
      }
    }
  ...
  // Handle suggest vendors - compose message from form data and auto-send
  const handleSuggestVendors = () => {
    const data = documentType === 'RFP' ? currentChat?.rfpData : currentChat?.rfqData
    if (!data) return

    const parts = []

    if (documentType === 'RFP') {
      parts.push(` Can you fetch me any vendors for ${data.projectTitle || 'this project'}.`)
      if (data.scope) parts.push(`Scope: ${data.scope}`)
      if (data.mandatoryRequirements) parts.push(`Requirements: ${data.mandatoryRequirements}`)
      if (data.submissionDeadline) parts.push(`Deadline: ${data.submissionDeadline}`)
      if (data.evaluationBasis) parts.push(`Evaluation: ${data.evaluationBasis}`)
    } else {
      parts.push(` Can you fetch me any vendors for ${data.procurementType || 'this procurement'}.`)
      if (data.requirementSummary) parts.push(`Requirement: ${data.requirementSummary}`)
      if (data.budgetRange) parts.push(`Budget: ${data.budgetRange}`)
      if (data.deliveryTimeline) parts.push(`Timeline: ${data.deliveryTimeline}`)
      if (data.quantity) parts.push(`Quantity: ${data.quantity}`)
    }

    const message = parts.join(' ')
    hideDetailPanel()
    setPendingChatMessage({ message, autoSend: true })
  }
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* Action Buttons */}
      <div className="p-4 border-t border-lyzr-cream bg-white/80 backdrop-blur-sm flex gap-2">
        <Button
          variant="primary"
          className="flex-1"
          onClick={handleSend}
